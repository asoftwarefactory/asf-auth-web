...
class AppAuthWebPlugin extends FlutterAppAuthPlatform {
  ...
  Future<AuthorizationResponse?> authorize(AuthorizationRequest request) async {
    ...
    try {
      if (request.promptValues != null &&
        ...
        // openPopUp(authUri, 'auth', 640, 600, true);
  ...
  static Future<AuthorizationTokenResponse> exchangeCode(
    ...
    if (clientSecret == null) {
      response = await http.post(Uri.parse(tokenEndpoint), body: {
        "client_id": clientId,
  ...
  static Future<AuthorizationServiceConfiguration> getConfiguration(
    ...
    if (response.statusCode != 200)
      throw UnsupportedError(_DISCOVERY_ERROR_MESSAGE_FORMAT.replaceAll(
          "%2", response.reasonPhrase ?? "Unknown Error"));
  ...
  static Future<String> openPopUp(
    ...
    html.window.onMessage.first.then((event) {
      ...
      print(url);
      callbackUrlCompleter.complete(url);
      child.close();
    });

    //This handles the user closing the window without a response
    while (!callbackUrlCompleter.isCompleted) {
      await Future.delayed(Duration(milliseconds: 500));
      if ((child.closed ?? false) && !callbackUrlCompleter.isCompleted)
        callbackUrlCompleter.completeError(StateError('User Closed'));

      if (callbackUrlCompleter.isCompleted) break;
    }

    return callbackUrlCompleter.future;
  }

  static Future<String> loadAuthPage(String authUri) async {
    html.window.location.assign(authUri);
    ...
    while (!callbackUrlCompleter.isCompleted) {
      await Future.delayed(Duration(milliseconds: 500));
      if (!callbackUrlCompleter.isCompleted)
        callbackUrlCompleter.completeError(StateError('User Closed'));
  ...
  static Future<String> openPopup(String url, String name) async {
    _loginPopup = html.window
        .open(url, "Google Auth", "width=800, height=900, scrollbars=yes");
  ...
  static Future<String> getAuthUri(AuthorizationRequest request) async {
    final serviceConfiguration = await getConfiguration(
        request.serviceConfiguration, request.discoveryUrl, request.issuer);
